...
# GUI POP-UP WITH REQUIREMENTS

# use SimplePyGui to allow user to select the folder where they have saved the data for this script
source_dir_of_required_files = sg.popup_get_folder(message="Welcome! For this script to run, please select the "
...
msg = ("Before proceeding, please ensure the following:\n\n"
       ...
       "your sequenced samples and the following column headers (see example):\n "
       "'sample' (i.e. name of sample) , 'age', 'sex', 'status' (i.e. case or control)\n\n"
       " Click \"OK\" if you have ensured all of the above\n\n")
image = os.path.join(source_dir_of_required_files, "sample_info_example.png")

title = 'Please Confirm'

if sg.popup_ok(msg, title=title, image=image, keep_on_top=False):
    pass
else:
    exit()
...
def fishers_test(df_list, row_list):
    """"
    Applies the fishers exact function to every dataframe in a list and adds the odd-ratios and p-values to an pandas dataframe.
...
pathogenic_variants_file_location = os.path.join(source_dir_of_required_files,
                                                 "mutpred_mtoolbox_apogee_cheat_sheet.csv")
...
# use SimplePyGui to allow user to select the folder with their haplogroup.txt files generated by mtDNA-Server
source_dir_of_mtdnaserver_variant_files = sg.popup_get_folder(
    message='Please select the folder in which you have saved '
            'all your mtDNA-Server "haplogroups.txt" files',
...
# use path chosen by user to the "Required_files" folder to open the file
phylotree_file_location = os.path.join(source_dir_of_required_files, "phylotree_17.txt")
...
# use path chosen by user to the "Required_files" folder to open the file
mitomap_haplo_markers_file_location = os.path.join(source_dir_of_required_files, "mitomap_haplogroup_markers_80%.csv")
...
# use SimplePyGui to allow user to select the folder with their mitomaster_output files
source_dir_of_mitomaster_files = sg.popup_get_folder(message='Please select the folder in which you have saved '
                                                             'all your Mitomaster input and output files',
                                                     title='Select a folder')
...
if len(glob.glob('*mitomaster*.csv')) == 0:
    ...
    sg.popup_quick_message('OOPS. Looks like there aren\'t any mitomaster files in your selected folder\n\n'
                           'Please check:\n\n'
...
# else make one dataframe from individual mitomaster*.csv files
    ...
    all_mitomaster_files = glob.glob(os.path.join(source_dir_of_mitomaster_files, '*mitomaster*.csv'))
...
mitomaster['haplo_seq'] = mitomaster['haplo_seq'].str.replace(")", "")
# split "GB_FL_freq%" to drop (CR): the control region sequences
mitomaster['GB_FL_freq%'] = mitomaster['GB_FL_freq%'].str.replace('(%).*', '')
# split "GB_FL_seq" to drop (CR)
mitomaster['GB_FL_seq'] = mitomaster['GB_FL_seq'].str.replace(r"\(.*", "")
# remove % from column
mitomaster['conservation%'] = mitomaster['conservation%'].str.replace('(%).*', '')
...
layout = [[sg.Text("Please enter the heteroplasmy level you'd like to include in your analysis greater than 50.")],
          [sg.InputText()],
          [sg.Submit(), sg.Cancel()]]
...
# use SimplePyGui to allow user to select the folder with their sample_info files
source_dir_of_sample_info_files = sg.popup_get_folder(message=" Please select the folder in which you have saved "
...
# define layout
layout = [
    [sg.Text('Which mitochondrial haplogroups would you like to include in the analysis?', size=(65, 2),
             font=('Arial', 12), justification='left')],
    [sg.Listbox(values=['African', 'European'], font=('Arial', 12), select_mode='extended', key='ancestry',
                size=(30, 2))],
    [sg.Button('Select', font=('Arial', 12))]]
# Define Window
win = sg.Window('Please select', layout)
# Read  values entered by user
event, values = win.read()
# access the selected value in the list box and save it as a varaible
reply = values['ancestry']
# close first window
win.close()
...
samples_removed = samples_removed.loc[
    (samples_removed['haplogroup_quality'] <= 90) | (samples_removed['haplogroup_quality'] > 100) | (
        ~samples_removed['haplogrep_haplogroup'].str.contains('|'.join(haplo_choice))), ['sample',
                                                                                         'haplogrep_haplogroup',
                                                                                         'haplogroup_quality', 'status',
                                                                                         'sex', 'age']]
samples_removed = samples_removed.drop_duplicates(subset='sample', keep='first')

# use SimplePyGui to allow user to select the folder where they would like to save their output files to
dir_of_analysis_files = sg.popup_get_folder(
    message='Please select the folder in which to save your analysis output files',
    title='Select a folder')
# change the directory to the one selected by the user
os.chdir(dir_of_analysis_files)

# make a txt file with info of samples removed
samples_removed.to_csv('excluded_samples.txt', sep='\t', index=False)

# Filter mitomaster output for samples with haplogroup quality >90 and a haplogroup == to the lineage selected by the user
mitomaster_output = mitomaster_output.loc[
    ((mitomaster_output['haplogroup_quality'] > 90) & (mitomaster_output['haplogroup_quality'] <= 100)) &
...
# make a new df with common out-of-place coding_CIII variants
...
# make a new df with common out-of-place coding_CIV variants
...
# make a new df with common out-of-place coding_CV variants
...
# make a new df with common out-of-place syn variants
...
# make a new df with common out-of-place syn_CI variants
...
# make a new df with common out-of-place syn_CIII variants
...
# make a new df with common out-of-place syn_CIV variants
...
# make a new df with common out-of-place syn_CV variants
...
# make a new df with common out-of-place nonsyn variants
...
# make a new df with common out of place variants that have been scored with one of the three in silico tools as mildy deletrious
common_scored_nonsyn_oxphos_var = rare_variants_df(nonsyn_oxphos_var, 'scored_variant', sample_list,
                                                   sample_info_with_haplogroups)
...
common_variants = pd.concat(
    [common_mtDNA_var_cross_tab_graph, common_rRNA_var_cross_tab_graph, common_tRNA_var_cross_tab_graph,
     common_noncoding_var_cross_tab_graph, common_coding_var_cross_tab_graph], ignore_index=True, sort=False)
common_variants = common_variants.astype({"percentage": float})

# specify values
common_variants_plot = sns.barplot(x='rare_var', y='percentage', hue='status', data=common_variants, edgecolor="grey",
                                   linewidth=1)
...
scored_common_nonsyn_variants_plot.set_ylabel('Percentage of individuals (%)', fontsize=12, weight='bold',
                                              labelpad=18)
scored_common_nonsyn_variants_plot.set_title(
    'Percentage of individuals with scored, non-synonymous out-of-place variants',
    fontsize=14, weight='bold',
    pad=18)
...
# Find the number of individuals from each status group who have out-of-place mutpred-scored variants grouped by count
...
common_nonsyn_var_mtoolbox_variant_load, common_nonsyn_var_mtoolbox_count = \
    pathogenic_scoring(dataframe=common_nonsyn_var, pathogenic_cutoff_value=0.4311,
                       pathogenic_col_name='mtoolbox_ds')
# Find the number of individuals from each status group who have out-of-place mtoolbox-scored variants grouped by count
...
# Find the number of individuals from each status group who have out-of-place apogee-scored variants grouped by count
...
# Make one excel sheet with all the out-of-place counts used for graphs
...
nonsyn_var_mutpred_variant_load_filter = nonsyn_var_mutpred_variant_load_filter.merge(sample_info_with_haplogroups,
                                                                                      on='sample')
nonsyn_var_mtoolbox_variant_load_filter = nonsyn_var_mtoolbox_variant_load.filter(['sample', 'variant_load'])
nonsyn_var_mtoolbox_variant_load_filter = nonsyn_var_mtoolbox_variant_load_filter.merge(sample_info_with_haplogroups,
                                                                                        on='sample')
nonsyn_var_apogee_variant_load_filter = nonsyn_var_apogee_variant_load.filter(['sample', 'variant_load'])
nonsyn_var_apogee_variant_load_filter = nonsyn_var_apogee_variant_load_filter.merge(sample_info_with_haplogroups,
                                                                                    on='sample')
...
# trim dataframes to only include 2 columns, then add on sample info columns
common_nonsyn_var_mutpred_variant_load_filter = common_nonsyn_var_mutpred_variant_load.filter(
    ['sample', 'variant_load'])
common_nonsyn_var_mutpred_variant_load_filter = common_nonsyn_var_mutpred_variant_load_filter.merge(
    sample_info_with_haplogroups, on='sample')
common_nonsyn_var_apogee_variant_load_filter = common_nonsyn_var_apogee_variant_load.filter(['sample', 'variant_load'])
common_nonsyn_var_apogee_variant_load_filter = common_nonsyn_var_apogee_variant_load_filter.merge(
    sample_info_with_haplogroups, on='sample')
common_nonsyn_var_mtoolbox_variant_load_filter = common_nonsyn_var_mtoolbox_variant_load.filter(
    ['sample', 'variant_load'])
common_nonsyn_var_mtoolbox_variant_load_filter = common_nonsyn_var_mtoolbox_variant_load_filter.merge(
    sample_info_with_haplogroups, on='sample')
...
common_nonsyn_var_mutpred_count_filter = common_nonsyn_var_mutpred_count_filter.merge(sample_info_with_haplogroups,
                                                                                      on='sample')
common_nonsyn_var_apogee_count_filter = common_nonsyn_var_apogee_count.filter(['sample', 'pathogenic_count'])
common_nonsyn_var_apogee_count_filter = common_nonsyn_var_apogee_count_filter.merge(sample_info_with_haplogroups,
                                                                                    on='sample')
common_nonsyn_var_mtoolbox_count_filter = common_nonsyn_var_mtoolbox_count.filter(['sample', 'pathogenic_count'])
common_nonsyn_var_mtoolbox_count_filter = common_nonsyn_var_mtoolbox_count_filter.merge(sample_info_with_haplogroups,
                                                                                        on='sample')
...
# The above spreadsheet gives a list of all individuals with a count of *out-of-place* scored
# variants predicted to be mildy deleterious in their mtDNA
# It is filtered to contain only out-of-place variants
...
save_xls_fishers(dict_df=private_var_fishers_dict, path='OoP_Fishers_test_input.xls')

# LIST OF COMMON OUT-OF-PLACE VARIANTS OUTPUT
# ------------------------------
...
save_xls(dict_df=rare_var_dict, path='common_OoP_variants_output.xls')
...
common_mtDNA_var_fishers_tests = fishers_test(df_list=common_mtDNA_var_df_list,
                                              row_list=Fisher_test_DataFrame_list_scored)

# save fishers exact output as xlsx file
common_mtDNA_var_fishers_tests.to_excel("OoP_Fishers_test_output.xlsx", index=False)
